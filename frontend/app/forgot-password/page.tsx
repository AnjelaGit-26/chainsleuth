...
import { Terminal, ArrowRight, Lock, Loader2, AlertCircle, Mail, CheckCircle2, ArrowLeft } from "lucide-react";
...
export default function ForgotPasswordPage() {
  ...
  const handleResetRequest = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        console.error("[ForgotPasswordPage] Supabase error:", error.message);
        setErrorMsg("Unable to process password reset request. Please try again.");
      } else {
        setInfoMsg("If the email is registered, a password reset link has been sent. Please check your inbox.");
      }
    } catch (err: unknown) {
      console.error("[ForgotPasswordPage] Unexpected error:", err);
      const msg = err instanceof Error ? err.message : "Service error processing password reset.";
      setErrorMsg(msg);
    } finally {
      ...
      <div className="w-full max-w-xl bg-slate-950 border border-slate-800 rounded p-6 shadow-2xl space-y-6">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-sm font-bold tracking-wider uppercase font-sans text-slate-100">
                CHAINSLEUTH // PASSWORD RECOVERY
              </div>
              <div className="text-[10px] text-slate-500 uppercase tracking-widest">
                Crypto Fraud Investigation System • Account Recovery
              </div>
            </div>
          </div>
        ...
        {infoMsg && (
          <div className="p-3.5 bg-emerald-950/90 border border-emerald-700 rounded text-emerald-200 text-xs font-mono space-y-1 shadow-lg">
            ...
            <p className="text-[11px] text-emerald-300 leading-relaxed font-mono">{infoMsg}</p>
          </div>
        )}
        ...
        <form onSubmit={handleResetRequest} className="space-y-4 font-sans">
          ...
          <div className="space-y-1.5 font-sans">
            <label className="text-xs text-slate-300 font-semibold flex items-center gap-1.5 font-mono">
              ...
              <span>REGISTERED OFFICER EMAIL</span>
            </label>
            <input
              type="email"
              required
          ...
          <div className="text-center pt-2 text-xs font-sans">
            <Link href="/login" className="text-slate-400 hover:text-slate-200 flex items-center justify-center gap-1.5">
        ...
        <div className="flex items-center justify-between text-[10px] text-slate-500 border-t border-slate-800/80 pt-4 font-mono">
          <div>SIH26183 • SUPABASE AUTH RECOVERY</div>
